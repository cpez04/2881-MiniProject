...
class FirewallVisualizer:
    ...
    def plot_bl_vs_trigger_rate(self, save_path: str = None, figsize=(10, 6)):
        """
        Plot Blocked-Leakage vs Trigger Rate (alternative to BL vs FPR).
        More informative when leakage rate is very high.
        ...
        for detector_name in self.detector_names:
            ...
            # Plot main line
            ax.plot(tr, bl, label=detector_name,
                   color=colors.get(detector_name, 'gray'),
                   marker=markers.get(detector_name, 'o'),
                   markersize=6, linewidth=2, alpha=0.8)

            # Add confidence intervals
            if 'blocked_leakage_ci' in df.columns:
                bl_ci = np.array([ci for ci in df['blocked_leakage_ci']])
                ax.fill_between(tr, bl_ci[:, 0], bl_ci[:, 1],
                               alpha=0.15, color=colors.get(detector_name, 'gray'))

        # Target lines
        ax.axhline(0.7, color='red', linestyle='--', alpha=0.5, label='BL ≥ 70% target')

        ax.set_xlabel('Trigger Rate (% of outputs blocked)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Blocked Leakage (% of leaks caught)', fontsize=12, fontweight='bold')
        ax.set_title('Firewall Trade-off: Leakage Prevention vs Output Blocking',
                    fontsize=14, fontweight='bold')
        ax.legend(loc='lower right', fontsize=10)
        ax.grid(True, alpha=0.3)
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Saved BL vs TR plot to {save_path}")
...
def main():
    ...
    viz.plot_bl_vs_trigger_rate(
        save_path=output_dir / f"{model_name}_bl_vs_trigger_rate.png"
    )
